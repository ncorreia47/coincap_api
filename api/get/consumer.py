...
from utils.load_environment import LoadEnvironment
from utils.load_to_database import Database
from utils.basic_data_transformation import NormalizeData


class Consumer:
    """
    Classe responsável por consumir os dados de API
    """

    def get_api_data(env_name: str, endpoint: str):
        print(f'Carregando o ambiente: {env_name}')
        LoadEnvironment().load_environment(env_name)
        ...
        try:
            ...
            for req in range(int(num_requests)):
                ...
                if int(payload) <= int(max_payload_per_request):
                    ...
                    dataframe.columns = NormalizeData.normalize_column_names(dataframe.columns)

                    Database().load_dataframe(dataframe=dataframe, 
                                            context=endpoint, 
                                            env_name=env_name, 
                                            schema = 'coincap_api')
